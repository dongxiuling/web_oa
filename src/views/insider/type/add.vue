...
<script>
import { saveOutType, updateOutType, getOutTypeById } from '@/api/insider'
export default {
  data() {
    return {
      cate: {
  ...
  methods: {
    ...
    // 添加会场
    async addHandle() {
      const res = await saveOutType(this.cate)
      if (res && res.code === '200') {
        this.$message({
          message: '添加成功',
          type: 'success'
    ...
    async updateHandle() {
      const res = await updateOutType(this.cate)
      // console.log(res);
      if (res && res.code === '200') {
        this.$message({
          message: '修改成功',
  ...
  async mounted() {
    ...
    if (id) { // 修改
      const res = await getOutTypeById(id)
      console.log(res)
      if (res && res.code === '200') {
        this.cate = res.data
      }
